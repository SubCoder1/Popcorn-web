...
<template>
  ...
  >
    <div class="loader m-auto" v-show="gang_search.load_search"></div>
    <div
...
export default {
  ...
  methods: {
    ...
    validatePassKey: function () {
      if (
        this.gang_join.gang_pass_key.length > 0 &&
        this.gang_join.gang_pass_key.length < 5
      ) {
        this.gang_join.passKeyErr = true;
        return false;
      }
    ...
    togglePassKeyModal: async function (gang_join_name, gang_join_admin, gang_is_private) {
      ...
      if (!gang_is_private) {
        await this.joinGang(false);
      } else if (!this.showPassKeyModal) {
        this.load_btn = false;
      }
    },
